...
for i in range(N):
    ...
    # Generate problem (x) from concepts (c)
    # Include instructions to generate a math problem and put answer in \boxed{}
...
Problem:"""
    ...
    # Retry logic for empty problems
    max_retries = 2
    x = ""
    for attempt in range(max_retries):
        ...
        with torch.no_grad():
            ...
            output_ids = prompt_model.generate(
                **inputs,
                max_new_tokens=1024,  # Generate enough tokens for complete problems
                do_sample=True,
                temperature=0.7,
                top_p=0.9,
        ...
        # If we got a non-empty problem, break out of retry loop
        if x and len(x.strip()) > 20:  # Minimum reasonable length
            break
        
        # If this was the last attempt, keep what we have (even if empty)
    ...
    if x and len(x.strip()) > 0:
        ...
        print(f"Skipping {i+1}/{N}: empty problem generated after {max_retries} attempts")
